...
from processing.RL.hyperparameters import max_range, state_dim, action_dim, actor_layers, critic_layers, \
...
for i in range(epochs):
    ...
    while t <= max_time_steps and retval == 0:
        ...
        print(f"#{i+1}-{step+1} - T: {t:.2f}, IT: {t - tp:.2f}, E: {E:.2f}, d: {np.linalg.norm(np.array(list(global_state[19:22]))) - safe_sphere:.2f},"
              f" R: {total_reward:.2f}, ik_success: {success}, action: {noise} + {not_scaled_action}, p_des: {TD[:3, -1]}")
        if retval != 0:
            break
    ...
    agent.update(update_iteration=200, gamma=gamma, tau=tau, batch_size=batch_size)
    if i % 10 == 0:
